...
def main(params):
    ...
    [_, _, _, _, _, _, all_word_level_stroke_in, all_word_level_stroke_out, all_word_level_stroke_length,
     ...
     all_segment_level_stroke_out, all_segment_level_stroke_length, all_segment_level_term, all_segment_level_char,
     all_segment_level_char_length] = dl.next_batch(TYPE='TRAIN', uid=params.writer_id,
                                                    tids=[i for i in range(params.num_samples)])
        ...
        with torch.no_grad():
            commands_list = net.sample(
                [batch_word_level_stroke_in, batch_word_level_stroke_out, batch_word_level_stroke_length,
                 ...
                 batch_segment_level_stroke_in, batch_segment_level_stroke_out, batch_segment_level_stroke_length,
                 batch_segment_level_term, batch_segment_level_char, batch_segment_level_char_length])
    ...
    def sample_word2(target_word):
        ...
        for sid, sentence in enumerate(all_segment_level_char[0]):
            for wid, word in enumerate(sentence):
                ...
                if segment in available_segments:
                    available_segments[segment].append(
                        [all_segment_level_stroke_in[0][sid][wid][:all_segment_level_stroke_length[0][sid][wid]],
                         all_segment_level_stroke_out[0][sid][wid][:all_segment_level_stroke_length[0][sid][wid]],
                         split_ids])
                else:
                    available_segments[segment] = [
        ...
        while index <= len(target_word):
            ...
            for end_index in range(len(target_word), index, -1):
                ...
                if segment in available_segments:
                    ...
                    available = True
                    candidates = available_segments[segment]
                    segment_level_stroke_in, segment_level_stroke_out, split_ids = candidates[
            ...
            if not available:
                ...
                out = net.char_vec_fc2_1(out)
                char_matrix = out.view([-1, 256, 256])
                TYPE_A_WC = torch.bmm(char_matrix,
                                      mean_global_W.repeat(char_matrix.size(0), 1).unsqueeze(2)).squeeze().unsqueeze(0)
                index += 1
