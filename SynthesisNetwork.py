...
class SynthesisNetwork(nn.Module):
    def __init__(self, weight_dim=512, num_layers=3, scale_sd=1, clamp_mdn=0, sentence_loss=True, word_loss=True,
                 segment_loss=True, TYPE_A=True, TYPE_B=True, TYPE_C=True, TYPE_D=True, ORIGINAL=True, REC=True):
        ...
        self.gen_state_lstm2 = nn.LSTM(self.weight_dim * 2, self.weight_dim * 2, batch_first=True,
    ...
    def forward(self, inputs):
        [sentence_level_stroke_in, sentence_level_stroke_out, sentence_level_stroke_length, sentence_level_term,
         sentence_level_char, sentence_level_char_length, word_level_stroke_in, word_level_stroke_out,
         word_level_stroke_length, word_level_term, word_level_char, word_level_char_length, segment_level_stroke_in,
        ...
        for uid in range(len(sentence_level_stroke_in)):
            if self.sentence_loss:
                ...
                for sentence_batch_id in range(sentence_batch_size):
                    ...
                    char_vector = torch.eye(len(CHARACTERS))[
                        user_sentence_level_char[sentence_batch_id][:curr_char_len]].to(self.device)
                ...
                sentence_Ws_consistency_loss = torch.mean(torch.mean(
                    torch.mul(sentence_W_mean_repeat - sentence_Ws_reshaped,
                              sentence_W_mean_repeat - sentence_Ws_reshaped), -1))
                ...
                for sentence_batch_id in range(sentence_batch_size):
                    ...
                    sentence_level_gen_encoded = sentence_gen_state_out[sentence_batch_id][
                                                 :user_sentence_level_stroke_length[sentence_batch_id][0]]
                    sentence_level_target_eos = user_sentence_level_stroke_out[sentence_batch_id][
                                                :user_sentence_level_stroke_length[sentence_batch_id][0]][:, 2]
                    sentence_level_target_x = user_sentence_level_stroke_out[sentence_batch_id][
                                              :user_sentence_level_stroke_length[sentence_batch_id][0]][:, 0:1]
                    sentence_level_target_y = user_sentence_level_stroke_out[sentence_batch_id][
                                              :user_sentence_level_stroke_length[sentence_batch_id][0]][:, 1:2]
                    sentence_level_target_term = user_sentence_level_term[sentence_batch_id][
                                                 :user_sentence_level_stroke_length[sentence_batch_id][0]]
                    ...
                    if self.ORIGINAL:
                        ...
                        sentence_gen_lstm2_in_ORIGINAL = torch.cat([sentence_level_gen_encoded, sentence_Wc_t_ORIGINAL],
                                                                   -1)
                        ...
                        [mu1_ORIGINAL, mu2_ORIGINAL, sig1_ORIGINAL, sig2_ORIGINAL, rho_ORIGINAL,
                         pi_ORIGINAL] = torch.split(mdn_out_ORIGINAL[:, 1:], self.num_mixtures, 1)
                        ...
                        gaussian_ORIGINAL = gaussian_2d(sentence_level_target_x, sentence_level_target_y, mu1_ORIGINAL,
                                                        mu2_ORIGINAL, sig1_ORIGINAL, sig2_ORIGINAL, rho_ORIGINAL)
                        loss_gaussian_ORIGINAL = - torch.log(torch.sum(pi_ORIGINAL * gaussian_ORIGINAL, dim=1) + 1e-5)

                        ORIGINAL_sentence_term_loss = self.term_bce_loss(term_out_ORIGINAL.squeeze(1),
                                                                         sentence_level_target_term)
                        ORIGINAL_sentence_loc_loss = torch.mean(loss_gaussian_ORIGINAL)
                        ORIGINAL_sentence_touch_loss = self.mdn_bce_loss(eos_ORIGINAL.squeeze(1),
                                                                         sentence_level_target_eos)
                    ...
                    if self.TYPE_A:
                        ...
                        sentence_Wc_rec_TYPE_A = torch.bmm(sentence_C1, \
                                                           sentence_W_mean.repeat(sentence_C1.size(0), 1).unsqueeze(
                                                               2)).squeeze(-1)
                        ...
                        sentence_gen_lstm2_in_TYPE_A = torch.cat([sentence_level_gen_encoded, sentence_Wc_t_rec_TYPE_A],
                                                                 -1)
                        ...
                        [mu1_TYPE_A, mu2_TYPE_A, sig1_TYPE_A, sig2_TYPE_A, rho_TYPE_A, pi_TYPE_A] = torch.split(
                            mdn_out_TYPE_A[:, 1:], self.num_mixtures, 1)
                        ...
                        gaussian_TYPE_A = gaussian_2d(sentence_level_target_x, sentence_level_target_y, mu1_TYPE_A,
                                                      mu2_TYPE_A, sig1_TYPE_A, sig2_TYPE_A, rho_TYPE_A)
                        loss_gaussian_TYPE_A = - torch.log(torch.sum(pi_TYPE_A * gaussian_TYPE_A, dim=1) + 1e-5)

                        TYPE_A_sentence_term_loss = self.term_bce_loss(term_out_TYPE_A.squeeze(1),
                                                                       sentence_level_target_term)
                    ...
                    if self.TYPE_B:
                        ...
                        sentence_W_c_TYPE_B_RAW = torch.bmm(unique_char_matrices_1,
                                                            sentence_W_mean.repeat(unique_char_matrices_1.size(0),
                                                                                   1).unsqueeze(2)).squeeze(-1)
                        ...
                        sentence_gen_lstm2_in_TYPE_B = torch.cat([sentence_level_gen_encoded, sentence_Wc_t_rec_TYPE_B],
                                                                 -1)
                        ...
                        [mu1_TYPE_B, mu2_TYPE_B, sig1_TYPE_B, sig2_TYPE_B, rho_TYPE_B, pi_TYPE_B] = torch.split(
                            mdn_out_TYPE_B[:, 1:], self.num_mixtures, 1)
                        ...
                        gaussian_TYPE_B = gaussian_2d(sentence_level_target_x, sentence_level_target_y, mu1_TYPE_B,
                                                      mu2_TYPE_B, sig1_TYPE_B, sig2_TYPE_B, rho_TYPE_B)
                        loss_gaussian_TYPE_B = - torch.log(torch.sum(pi_TYPE_B * gaussian_TYPE_B, dim=1) + 1e-5)

                        TYPE_B_sentence_term_loss = self.term_bce_loss(term_out_TYPE_B.squeeze(1),
                                                                       sentence_level_target_term)
                ...
                if self.ORIGINAL:
                    ALL_ORIGINAL_sentence_termination_loss.append(
                        torch.mean(torch.stack(ORIGINAL_sentence_termination_loss)))
                    ALL_ORIGINAL_sentence_loc_reconstruct_loss.append(
                        torch.mean(torch.stack(ORIGINAL_sentence_loc_reconstruct_loss)))
                    ALL_ORIGINAL_sentence_touch_reconstruct_loss.append(
                        torch.mean(torch.stack(ORIGINAL_sentence_touch_reconstruct_loss)))

                if self.TYPE_A:
                    ALL_TYPE_A_sentence_termination_loss.append(
                        torch.mean(torch.stack(TYPE_A_sentence_termination_loss)))
                    ALL_TYPE_A_sentence_loc_reconstruct_loss.append(
                        torch.mean(torch.stack(TYPE_A_sentence_loc_reconstruct_loss)))
                    ALL_TYPE_A_sentence_touch_reconstruct_loss.append(
                        torch.mean(torch.stack(TYPE_A_sentence_touch_reconstruct_loss)))
                    ...
                    if self.REC:
                        ...
                        for sentence_batch_id in range(len(sentence_Wc_rec_TYPE_)):
                            ...
                            sentence_WC_reconstruct_loss_TYPE_A = torch.mean(torch.mean(
                                torch.mul(sentence_Wc_ORIGINAL - sentence_Wc_TYPE_A,
                                          sentence_Wc_ORIGINAL - sentence_Wc_TYPE_A), -1))
                            TYPE_A_sentence_WC_reconstruct_loss.append(sentence_WC_reconstruct_loss_TYPE_A)
                        ALL_TYPE_A_sentence_WC_reconstruct_loss.append(
                            torch.mean(torch.stack(TYPE_A_sentence_WC_reconstruct_loss)))

                if self.TYPE_B:
                    ALL_TYPE_B_sentence_termination_loss.append(
                        torch.mean(torch.stack(TYPE_B_sentence_termination_loss)))
                    ALL_TYPE_B_sentence_loc_reconstruct_loss.append(
                        torch.mean(torch.stack(TYPE_B_sentence_loc_reconstruct_loss)))
                    ALL_TYPE_B_sentence_touch_reconstruct_loss.append(
                        torch.mean(torch.stack(TYPE_B_sentence_touch_reconstruct_loss)))
                    ...
                    if self.REC:
                        ...
                        for sentence_batch_id in range(len(sentence_Wc_rec_TYPE_)):
                            ...
                            sentence_WC_reconstruct_loss_TYPE_B = torch.mean(torch.mean(
                                torch.mul(sentence_Wc_ORIGINAL - sentence_Wc_TYPE_B,
                                          sentence_Wc_ORIGINAL - sentence_Wc_TYPE_B), -1))
                            TYPE_B_sentence_WC_reconstruct_loss.append(sentence_WC_reconstruct_loss_TYPE_B)
                        ALL_TYPE_B_sentence_WC_reconstruct_loss.append(
                            torch.mean(torch.stack(TYPE_B_sentence_WC_reconstruct_loss)))
            ...
            if self.word_loss:
                ...
                for word_batch_id in range(word_batch_size):
                    ...
                    char_vector = torch.eye(len(CHARACTERS))[user_word_level_char[word_batch_id][:curr_char_len]].to(
                        self.device)
                ...
                word_Ws_consistency_loss = torch.mean(torch.mean(
                    torch.mul(word_Ws_reshaped_mean_repeat - word_Ws_reshaped,
                              word_Ws_reshaped_mean_repeat - word_Ws_reshaped), -1))
                ...
                for word_batch_id in range(word_batch_size):
                    ...
                    word_level_gen_encoded = word_gen_state_out[word_batch_id][
                                             :user_word_level_stroke_length[word_batch_id][0]]
                    word_level_target_eos = user_word_level_stroke_out[word_batch_id][
                                            :user_word_level_stroke_length[word_batch_id][0]][:, 2]
                    word_level_target_x = user_word_level_stroke_out[word_batch_id][
                                          :user_word_level_stroke_length[word_batch_id][0]][:, 0:1]
                    word_level_target_y = user_word_level_stroke_out[word_batch_id][
                                          :user_word_level_stroke_length[word_batch_id][0]][:, 1:2]
                    word_level_target_term = user_word_level_term[word_batch_id][
                                             :user_word_level_stroke_length[word_batch_id][0]]
                    ...
                    if self.ORIGINAL:
                        ...
                        gaussian_ORIGINAL = gaussian_2d(word_level_target_x, word_level_target_y, mu1_ORIGINAL,
                                                        mu2_ORIGINAL, sig1_ORIGINAL, sig2_ORIGINAL, rho_ORIGINAL)
                        loss_gaussian_ORIGINAL = - torch.log(torch.sum(pi_ORIGINAL * gaussian_ORIGINAL, dim=1) + 1e-5)

                        ORIGINAL_word_term_loss = self.term_bce_loss(term_out_ORIGINAL.squeeze(1),
                                                                     word_level_target_term)
                    ...
                    if self.TYPE_A:
                        ...
                        term_out_TYPE_A = self.term_fc3(term_out_TYPE_A)
                        term_pred_TYPE_A = self.term_sigmoid(term_out_TYPE_A)
                        gaussian_TYPE_A = gaussian_2d(word_level_target_x, word_level_target_y, mu1_TYPE_A, mu2_TYPE_A,
                                                      sig1_TYPE_A, sig2_TYPE_A, rho_TYPE_A)
                        loss_gaussian_TYPE_A = - torch.log(torch.sum(pi_TYPE_A * gaussian_TYPE_A, dim=1) + 1e-5)
                    ...
                    if self.TYPE_B:
                        ...
                        word_W_c_TYPE_B_RAW = torch.bmm(unique_char_matrices_1,
                                                        word_W_mean.repeat(unique_char_matrices_1.size(0), 1).unsqueeze(
                                                            2)).squeeze(-1)
                        ...
                        term_out_TYPE_B = self.term_fc3(term_out_TYPE_B)
                        term_pred_TYPE_B = self.term_sigmoid(term_out_TYPE_B)
                        gaussian_TYPE_B = gaussian_2d(word_level_target_x, word_level_target_y, mu1_TYPE_B, mu2_TYPE_B,
                                                      sig1_TYPE_B, sig2_TYPE_B, rho_TYPE_B)
                        loss_gaussian_TYPE_B = - torch.log(torch.sum(pi_TYPE_B * gaussian_TYPE_B, dim=1) + 1e-5)
                    ...
                    for segment_batch_id in range(segment_batch_size):
                        ...
                        char_vector = torch.eye(len(CHARACTERS))[
                            user_segment_level_char[segment_batch_id][:curr_char_len]].to(self.device)
                    ...
                    if self.segment_loss:
                        ...
                        segment_Ws_consistency_loss = torch.mean(torch.mean(
                            torch.mul(segment_Ws_reshaped_mean_repeat - segment_Ws_reshaped,
                                      segment_Ws_reshaped_mean_repeat - segment_Ws_reshaped), -1))
                        ...
                        for segment_batch_id in range(segment_batch_size):
                            segment_level_gen_encoded = segment_gen_state_out[segment_batch_id][
                                                        :user_segment_level_stroke_length[segment_batch_id][0]]
                            segment_level_target_eos = user_segment_level_stroke_out[segment_batch_id][
                                                       :user_segment_level_stroke_length[segment_batch_id][0]][:, 2]
                            segment_level_target_x = user_segment_level_stroke_out[segment_batch_id][
                                                     :user_segment_level_stroke_length[segment_batch_id][0]][:, 0:1]
                            segment_level_target_y = user_segment_level_stroke_out[segment_batch_id][
                                                     :user_segment_level_stroke_length[segment_batch_id][0]][:, 1:2]
                            segment_level_target_term = user_segment_level_term[segment_batch_id][
                                                        :user_segment_level_stroke_length[segment_batch_id][0]]
                            ...
                            if self.ORIGINAL:
                                ...
                                for i in range(user_segment_level_stroke_length[segment_batch_id][0]):
                                    segment_W_lstm_in_ORIGINAL.append(
                                        segment_Wc_rec_ORIGINAL[segment_batch_id][curr_id])
                                ...
                                segment_gen_lstm2_in_ORIGINAL = torch.cat(
                                    [segment_level_gen_encoded, segment_Wc_t_ORIGINAL], -1)
                                ...
                                gaussian_ORIGINAL = gaussian_2d(segment_level_target_x, segment_level_target_y,
                                                                mu1_ORIGINAL, mu2_ORIGINAL, sig1_ORIGINAL,
                                                                sig2_ORIGINAL, rho_ORIGINAL)
                                loss_gaussian_ORIGINAL = - torch.log(
                                    torch.sum(pi_ORIGINAL * gaussian_ORIGINAL, dim=1) + 1e-5)

                                ORIGINAL_segment_term_loss = self.term_bce_loss(term_out_ORIGINAL.squeeze(1),
                                                                                segment_level_target_term)
                                ORIGINAL_segment_loc_loss = torch.mean(loss_gaussian_ORIGINAL)
                                ORIGINAL_segment_touch_loss = self.mdn_bce_loss(eos_ORIGINAL.squeeze(1),
                                                                                segment_level_target_eos)
                            ...
                            if self.TYPE_A:
                                ...
                                segment_Wc_rec_TYPE_A = torch.bmm(segment_C1,
                                                                  segment_W_mean.repeat(segment_C1.size(0),
                                                                                        1).unsqueeze(2)).squeeze(-1)
                                ...
                                segment_gen_lstm2_in_TYPE_A = torch.cat(
                                    [segment_level_gen_encoded, segment_Wc_t_rec_TYPE_A], -1)
                                ...
                                term_out_TYPE_A = self.term_fc3(term_out_TYPE_A)
                                term_pred_TYPE_A = self.term_sigmoid(term_out_TYPE_A)
                                gaussian_TYPE_A = gaussian_2d(segment_level_target_x, segment_level_target_y,
                                                              mu1_TYPE_A, mu2_TYPE_A, sig1_TYPE_A, sig2_TYPE_A,
                                                              rho_TYPE_A)
                                loss_gaussian_TYPE_A = - torch.log(torch.sum(pi_TYPE_A * gaussian_TYPE_A, dim=1) + 1e-5)

                                TYPE_A_segment_term_loss = self.term_bce_loss(term_out_TYPE_A.squeeze(1),
                                                                              segment_level_target_term)
                                TYPE_A_segment_loc_loss = torch.mean(loss_gaussian_TYPE_A)
                                TYPE_A_segment_touch_loss = self.mdn_bce_loss(eos_TYPE_A.squeeze(1),
                                                                              segment_level_target_eos)
                            ...
                            if self.TYPE_B:
                                ...
                                segment_W_c_TYPE_B_RAW = torch.bmm(unique_char_matrices_1,
                                                                   segment_W_mean.repeat(unique_char_matrices_1.size(0),
                                                                                         1).unsqueeze(2)).squeeze(-1)
                                ...
                                segment_gen_lstm2_in_TYPE_B = torch.cat(
                                    [segment_level_gen_encoded, segment_Wc_t_rec_TYPE_B], -1)
                                ...
                                term_out_TYPE_B = self.term_fc3(term_out_TYPE_B)
                                term_pred_TYPE_B = self.term_sigmoid(term_out_TYPE_B)
                                gaussian_TYPE_B = gaussian_2d(segment_level_target_x, segment_level_target_y,
                                                              mu1_TYPE_B, mu2_TYPE_B, sig1_TYPE_B, sig2_TYPE_B,
                                                              rho_TYPE_B)
                                loss_gaussian_TYPE_B = - torch.log(torch.sum(pi_TYPE_B * gaussian_TYPE_B, dim=1) + 1e-5)

                                TYPE_B_segment_term_loss = self.term_bce_loss(term_out_TYPE_B.squeeze(1),
                                                                              segment_level_target_term)
                                TYPE_B_segment_loc_loss = torch.mean(loss_gaussian_TYPE_B)
                                TYPE_B_segment_touch_loss = self.mdn_bce_loss(eos_TYPE_B.squeeze(1),
                                                                              segment_level_target_eos)
                        ...
                        if self.ORIGINAL:
                            ALL_ORIGINAL_segment_termination_loss.append(
                                torch.mean(torch.stack(ORIGINAL_segment_termination_loss)))
                            ALL_ORIGINAL_segment_loc_reconstruct_loss.append(
                                torch.mean(torch.stack(ORIGINAL_segment_loc_reconstruct_loss)))
                            ALL_ORIGINAL_segment_touch_reconstruct_loss.append(
                                torch.mean(torch.stack(ORIGINAL_segment_touch_reconstruct_loss)))

                        if self.TYPE_A:
                            ALL_TYPE_A_segment_termination_loss.append(
                                torch.mean(torch.stack(TYPE_A_segment_termination_loss)))
                            ALL_TYPE_A_segment_loc_reconstruct_loss.append(
                                torch.mean(torch.stack(TYPE_A_segment_loc_reconstruct_loss)))
                            ALL_TYPE_A_segment_touch_reconstruct_loss.append(
                                torch.mean(torch.stack(TYPE_A_segment_touch_reconstruct_loss)))
                            ...
                            if self.REC:
                                ...
                                for segment_batch_id in range(len(segment_Wc_rec_ORIGINAL)):
                                    ...
                                    segment_WC_reconstruct_loss_TYPE_A = torch.mean(torch.mean(
                                        torch.mul(segment_Wc_ORIGINAL - segment_Wc_TYPE_A,
                                                  segment_Wc_ORIGINAL - segment_Wc_TYPE_A), -1))
                                    TYPE_A_segment_WC_reconstruct_loss.append(segment_WC_reconstruct_loss_TYPE_A)
                                ALL_TYPE_A_segment_WC_reconstruct_loss.append(
                                    torch.mean(torch.stack(TYPE_A_segment_WC_reconstruct_loss)))

                        if self.TYPE_B:
                            ALL_TYPE_B_segment_termination_loss.append(
                                torch.mean(torch.stack(TYPE_B_segment_termination_loss)))
                            ALL_TYPE_B_segment_loc_reconstruct_loss.append(
                                torch.mean(torch.stack(TYPE_B_segment_loc_reconstruct_loss)))
                            ALL_TYPE_B_segment_touch_reconstruct_loss.append(
                                torch.mean(torch.stack(TYPE_B_segment_touch_reconstruct_loss)))
                            ...
                            if self.REC:
                                ...
                                for segment_batch_id in range(len(segment_Wc_rec_ORIGINAL)):
                                    ...
                                    segment_WC_reconstruct_loss_TYPE_B = torch.mean(torch.mean(
                                        torch.mul(segment_Wc_ORIGINAL - segment_Wc_TYPE_B,
                                                  segment_Wc_ORIGINAL - segment_Wc_TYPE_B), -1))
                                    TYPE_B_segment_WC_reconstruct_loss.append(segment_WC_reconstruct_loss_TYPE_B)
                                ALL_TYPE_B_segment_WC_reconstruct_loss.append(
                                    torch.mean(torch.stack(TYPE_B_segment_WC_reconstruct_loss)))
                    ...
                    if self.TYPE_C:
                        ...
                        if len(word_Wc_rec_TYPE_C) == len(word_SPLITS[word_batch_id]):
                            ...
                            [mu1_TYPE_C, mu2_TYPE_C, sig1_TYPE_C, sig2_TYPE_C, rho_TYPE_C, pi_TYPE_C] = torch.split(
                                mdn_out_TYPE_C[:, 1:], self.num_mixtures, 1)
                            ...
                            gaussian_TYPE_C = gaussian_2d(word_level_target_x, word_level_target_y, mu1_TYPE_C,
                                                          mu2_TYPE_C, sig1_TYPE_C, sig2_TYPE_C, rho_TYPE_C)
                            loss_gaussian_TYPE_C = - torch.log(torch.sum(pi_TYPE_C * gaussian_TYPE_C, dim=1) + 1e-5)

                            TYPE_C_word_term_loss = self.term_bce_loss(term_out_TYPE_C.squeeze(1),
                                                                       word_level_target_term)
                    ...
                    if self.TYPE_D:
                        ...
                        if len(word_Wc_rec_TYPE_D) == len(word_SPLITS[word_batch_id]):
                            ...
                            [mu1_TYPE_D, mu2_TYPE_D, sig1_TYPE_D, sig2_TYPE_D, rho_TYPE_D, pi_TYPE_D] = torch.split(
                                mdn_out_TYPE_D[:, 1:], self.num_mixtures, 1)
                            ...
                            gaussian_TYPE_D = gaussian_2d(word_level_target_x, word_level_target_y, mu1_TYPE_D,
                                                          mu2_TYPE_D, sig1_TYPE_D, sig2_TYPE_D, rho_TYPE_D)
                            loss_gaussian_TYPE_D = - torch.log(torch.sum(pi_TYPE_D * gaussian_TYPE_D, dim=1) + 1e-5)

                            TYPE_D_word_term_loss = self.term_bce_loss(term_out_TYPE_D.squeeze(1),
                                                                       word_level_target_term)
                ...
                if self.ORIGINAL:
                    ...
                    ALL_ORIGINAL_word_loc_reconstruct_loss.append(
                        torch.mean(torch.stack(ORIGINAL_word_loc_reconstruct_loss)))
                    ALL_ORIGINAL_word_touch_reconstruct_loss.append(
                        torch.mean(torch.stack(ORIGINAL_word_touch_reconstruct_loss)))

                if self.TYPE_A:
                    ALL_TYPE_A_word_termination_loss.append(torch.mean(torch.stack(TYPE_A_word_termination_loss)))
                    ALL_TYPE_A_word_loc_reconstruct_loss.append(
                        torch.mean(torch.stack(TYPE_A_word_loc_reconstruct_loss)))
                    ALL_TYPE_A_word_touch_reconstruct_loss.append(
                        torch.mean(torch.stack(TYPE_A_word_touch_reconstruct_loss)))
                    ...
                    if self.REC:
                        ...
                        for word_batch_id in range(len(word_Wc_rec_ORIGINAL)):
                            ...
                            if len(word_Wc_ORIGINAL) == len(word_Wc_TYPE_A):
                                word_WC_reconstruct_loss_TYPE_A = torch.mean(torch.mean(
                                    torch.mul(word_Wc_ORIGINAL - word_Wc_TYPE_A, word_Wc_ORIGINAL - word_Wc_TYPE_A),
                                    -1))
                                TYPE_A_word_WC_reconstruct_loss.append(word_WC_reconstruct_loss_TYPE_A)
                        if len(TYPE_A_word_WC_reconstruct_loss) > 0:
                            ALL_TYPE_A_word_WC_reconstruct_loss.append(
                                torch.mean(torch.stack(TYPE_A_word_WC_reconstruct_loss)))

                if self.TYPE_B:
                    ALL_TYPE_B_word_termination_loss.append(torch.mean(torch.stack(TYPE_B_word_termination_loss)))
                    ALL_TYPE_B_word_loc_reconstruct_loss.append(
                        torch.mean(torch.stack(TYPE_B_word_loc_reconstruct_loss)))
                    ALL_TYPE_B_word_touch_reconstruct_loss.append(
                        torch.mean(torch.stack(TYPE_B_word_touch_reconstruct_loss)))
                    ...
                    if self.REC:
                        ...
                        for word_batch_id in range(len(word_Wc_rec_ORIGINAL)):
                            ...
                            if len(word_Wc_ORIGINAL) == len(word_Wc_TYPE_B):
                                word_WC_reconstruct_loss_TYPE_B = torch.mean(torch.mean(
                                    torch.mul(word_Wc_ORIGINAL - word_Wc_TYPE_B, word_Wc_ORIGINAL - word_Wc_TYPE_B),
                                    -1))
                                TYPE_B_word_WC_reconstruct_loss.append(word_WC_reconstruct_loss_TYPE_B)
                        if len(TYPE_B_word_WC_reconstruct_loss) > 0:
                            ALL_TYPE_B_word_WC_reconstruct_loss.append(
                                torch.mean(torch.stack(TYPE_B_word_WC_reconstruct_loss)))

                if self.TYPE_C:
                    ALL_TYPE_C_word_termination_loss.append(torch.mean(torch.stack(TYPE_C_word_termination_loss)))
                    ALL_TYPE_C_word_loc_reconstruct_loss.append(
                        torch.mean(torch.stack(TYPE_C_word_loc_reconstruct_loss)))
                    ALL_TYPE_C_word_touch_reconstruct_loss.append(
                        torch.mean(torch.stack(TYPE_C_word_touch_reconstruct_loss)))
                    ...
                    if self.REC:
                        ...
                        for word_batch_id in range(len(word_Wc_rec_ORIGINAL)):
                            ...
                            if len(word_Wc_ORIGINAL) == len(word_Wc_TYPE_C):
                                word_WC_reconstruct_loss_TYPE_C = torch.mean(torch.mean(
                                    torch.mul(word_Wc_ORIGINAL - word_Wc_TYPE_C, word_Wc_ORIGINAL - word_Wc_TYPE_C),
                                    -1))
                                TYPE_C_word_WC_reconstruct_loss.append(word_WC_reconstruct_loss_TYPE_C)
                        if len(TYPE_C_word_WC_reconstruct_loss) > 0:
                            ALL_TYPE_C_word_WC_reconstruct_loss.append(
                                torch.mean(torch.stack(TYPE_C_word_WC_reconstruct_loss)))

                if self.TYPE_D:
                    ALL_TYPE_D_word_termination_loss.append(torch.mean(torch.stack(TYPE_D_word_termination_loss)))
                    ALL_TYPE_D_word_loc_reconstruct_loss.append(
                        torch.mean(torch.stack(TYPE_D_word_loc_reconstruct_loss)))
                    ALL_TYPE_D_word_touch_reconstruct_loss.append(
                        torch.mean(torch.stack(TYPE_D_word_touch_reconstruct_loss)))
                    ...
                    if self.REC:
                        ...
                        for word_batch_id in range(len(word_Wc_rec_ORIGINAL)):
                            ...
                            if len(word_Wc_ORIGINAL) == len(word_Wc_TYPE_D):
                                word_WC_reconstruct_loss_TYPE_D = torch.mean(torch.mean(
                                    torch.mul(word_Wc_ORIGINAL - word_Wc_TYPE_D, word_Wc_ORIGINAL - word_Wc_TYPE_D),
                                    -1))
                                TYPE_D_word_WC_reconstruct_loss.append(word_WC_reconstruct_loss_TYPE_D)
                        if len(TYPE_D_word_WC_reconstruct_loss) > 0:
                            ALL_TYPE_D_word_WC_reconstruct_loss.append(
                                torch.mean(torch.stack(TYPE_D_word_WC_reconstruct_loss)))
                ...
                if self.segment_loss:
                    ...
                    if self.ORIGINAL:
                        SUPER_ALL_ORIGINAL_segment_termination_loss.append(
                            torch.mean(torch.stack(ALL_ORIGINAL_segment_termination_loss)))
                        SUPER_ALL_ORIGINAL_segment_loc_reconstruct_loss.append(
                            torch.mean(torch.stack(ALL_ORIGINAL_segment_loc_reconstruct_loss)))
                        SUPER_ALL_ORIGINAL_segment_touch_reconstruct_loss.append(
                            torch.mean(torch.stack(ALL_ORIGINAL_segment_touch_reconstruct_loss)))

                    if self.TYPE_A:
                        SUPER_ALL_TYPE_A_segment_termination_loss.append(
                            torch.mean(torch.stack(ALL_TYPE_A_segment_termination_loss)))
                        SUPER_ALL_TYPE_A_segment_loc_reconstruct_loss.append(
                            torch.mean(torch.stack(ALL_TYPE_A_segment_loc_reconstruct_loss)))
                        SUPER_ALL_TYPE_A_segment_touch_reconstruct_loss.append(
                            torch.mean(torch.stack(ALL_TYPE_A_segment_touch_reconstruct_loss)))
                        if self.REC:
                            SUPER_ALL_TYPE_A_segment_WC_reconstruct_loss.append(
                                torch.mean(torch.stack(ALL_TYPE_A_segment_WC_reconstruct_loss)))

                    if self.TYPE_B:
                        SUPER_ALL_TYPE_B_segment_termination_loss.append(
                            torch.mean(torch.stack(ALL_TYPE_B_segment_termination_loss)))
                        SUPER_ALL_TYPE_B_segment_loc_reconstruct_loss.append(
                            torch.mean(torch.stack(ALL_TYPE_B_segment_loc_reconstruct_loss)))
                        SUPER_ALL_TYPE_B_segment_touch_reconstruct_loss.append(
                            torch.mean(torch.stack(ALL_TYPE_B_segment_touch_reconstruct_loss)))
                        if self.REC:
                            SUPER_ALL_TYPE_B_segment_WC_reconstruct_loss.append(
                                torch.mean(torch.stack(ALL_TYPE_B_segment_WC_reconstruct_loss)))
        ...
        if self.sentence_loss:
            ...
            if self.ORIGINAL:
                mean_ORIGINAL_sentence_termination_loss = torch.mean(
                    torch.stack(ALL_ORIGINAL_sentence_termination_loss))
                mean_ORIGINAL_sentence_loc_reconstruct_loss = torch.mean(
                    torch.stack(ALL_ORIGINAL_sentence_loc_reconstruct_loss))
                mean_ORIGINAL_sentence_touch_reconstruct_loss = torch.mean(
                    torch.stack(ALL_ORIGINAL_sentence_touch_reconstruct_loss))
            if self.TYPE_A:
                mean_TYPE_A_sentence_termination_loss = torch.mean(torch.stack(ALL_TYPE_A_sentence_termination_loss))
                mean_TYPE_A_sentence_loc_reconstruct_loss = torch.mean(
                    torch.stack(ALL_TYPE_A_sentence_loc_reconstruct_loss))
                mean_TYPE_A_sentence_touch_reconstruct_loss = torch.mean(
                    torch.stack(ALL_TYPE_A_sentence_touch_reconstruct_loss))
                if self.REC:
                    mean_TYPE_A_sentence_WC_reconstruct_loss = torch.mean(
                        torch.stack(ALL_TYPE_A_sentence_WC_reconstruct_loss))
            if self.TYPE_B:
                mean_TYPE_B_sentence_termination_loss = torch.mean(torch.stack(ALL_TYPE_B_sentence_termination_loss))
                mean_TYPE_B_sentence_loc_reconstruct_loss = torch.mean(
                    torch.stack(ALL_TYPE_B_sentence_loc_reconstruct_loss))
                mean_TYPE_B_sentence_touch_reconstruct_loss = torch.mean(
                    torch.stack(ALL_TYPE_B_sentence_touch_reconstruct_loss))
                if self.REC:
                    mean_TYPE_B_sentence_WC_reconstruct_loss = torch.mean(
                        torch.stack(ALL_TYPE_B_sentence_WC_reconstruct_loss))
            ...
            sentence_losses = [total_sentence_loss, mean_sentence_W_consistency_loss,
                               mean_ORIGINAL_sentence_termination_loss, mean_ORIGINAL_sentence_loc_reconstruct_loss,
                               mean_ORIGINAL_sentence_touch_reconstruct_loss, mean_TYPE_A_sentence_termination_loss,
                               mean_TYPE_A_sentence_loc_reconstruct_loss, mean_TYPE_A_sentence_touch_reconstruct_loss,
                               mean_TYPE_B_sentence_termination_loss, mean_TYPE_B_sentence_loc_reconstruct_loss,
                               mean_TYPE_B_sentence_touch_reconstruct_loss, mean_TYPE_A_sentence_WC_reconstruct_loss,
        ...
        if self.word_loss:
            ...
            if self.ORIGINAL:
                ...
                mean_ORIGINAL_word_loc_reconstruct_loss = torch.mean(
                    torch.stack(ALL_ORIGINAL_word_loc_reconstruct_loss))
                mean_ORIGINAL_word_touch_reconstruct_loss = torch.mean(
                    torch.stack(ALL_ORIGINAL_word_touch_reconstruct_loss))
            if self.TYPE_A:
                ...
                mean_TYPE_A_word_touch_reconstruct_loss = torch.mean(
                    torch.stack(ALL_TYPE_A_word_touch_reconstruct_loss))
                if self.REC:
            ...
            if self.TYPE_B:
                ...
                mean_TYPE_B_word_touch_reconstruct_loss = torch.mean(
                    torch.stack(ALL_TYPE_B_word_touch_reconstruct_loss))
                if self.REC:
            ...
            if self.TYPE_C:
                ...
                mean_TYPE_C_word_touch_reconstruct_loss = torch.mean(
                    torch.stack(ALL_TYPE_C_word_touch_reconstruct_loss))
                if self.REC:
            ...
            if self.TYPE_D:
                ...
                mean_TYPE_D_word_touch_reconstruct_loss = torch.mean(
                    torch.stack(ALL_TYPE_D_word_touch_reconstruct_loss))
                if self.REC:
            ...
            word_losses = [total_word_loss, mean_word_W_consistency_loss, mean_ORIGINAL_word_termination_loss,
                           ...
                           mean_TYPE_C_word_WC_reconstruct_loss, mean_TYPE_D_word_WC_reconstruct_loss]
        ...
        if self.segment_loss:
            ...
            if self.ORIGINAL:
                mean_ORIGINAL_segment_termination_loss = torch.mean(
                    torch.stack(SUPER_ALL_ORIGINAL_segment_termination_loss))
                mean_ORIGINAL_segment_loc_reconstruct_loss = torch.mean(
                    torch.stack(SUPER_ALL_ORIGINAL_segment_loc_reconstruct_loss))
                mean_ORIGINAL_segment_touch_reconstruct_loss = torch.mean(
                    torch.stack(SUPER_ALL_ORIGINAL_segment_touch_reconstruct_loss))
            if self.TYPE_A:
                mean_TYPE_A_segment_termination_loss = torch.mean(
                    torch.stack(SUPER_ALL_TYPE_A_segment_termination_loss))
                mean_TYPE_A_segment_loc_reconstruct_loss = torch.mean(
                    torch.stack(SUPER_ALL_TYPE_A_segment_loc_reconstruct_loss))
                mean_TYPE_A_segment_touch_reconstruct_loss = torch.mean(
                    torch.stack(SUPER_ALL_TYPE_A_segment_touch_reconstruct_loss))
                if self.REC:
                    mean_TYPE_A_segment_WC_reconstruct_loss = torch.mean(
                        torch.stack(SUPER_ALL_TYPE_A_segment_WC_reconstruct_loss))
            if self.TYPE_B:
                mean_TYPE_B_segment_termination_loss = torch.mean(
                    torch.stack(SUPER_ALL_TYPE_B_segment_termination_loss))
                mean_TYPE_B_segment_loc_reconstruct_loss = torch.mean(
                    torch.stack(SUPER_ALL_TYPE_B_segment_loc_reconstruct_loss))
                mean_TYPE_B_segment_touch_reconstruct_loss = torch.mean(
                    torch.stack(SUPER_ALL_TYPE_B_segment_touch_reconstruct_loss))
                if self.REC:
                    mean_TYPE_B_segment_WC_reconstruct_loss = torch.mean(
                        torch.stack(SUPER_ALL_TYPE_B_segment_WC_reconstruct_loss))
            ...
            segment_losses = [total_segment_loss, mean_segment_W_consistency_loss,
                              mean_ORIGINAL_segment_termination_loss, mean_ORIGINAL_segment_loc_reconstruct_loss,
                              mean_ORIGINAL_segment_touch_reconstruct_loss, mean_TYPE_A_segment_termination_loss,
                              mean_TYPE_A_segment_loc_reconstruct_loss, mean_TYPE_A_segment_touch_reconstruct_loss,
                              mean_TYPE_B_segment_termination_loss, mean_TYPE_B_segment_loc_reconstruct_loss,
                              mean_TYPE_B_segment_touch_reconstruct_loss, mean_TYPE_A_segment_WC_reconstruct_loss,
                              mean_TYPE_B_segment_WC_reconstruct_loss]
